        ...
        <div className="p-8">
          <div className="flex justify-between items-center mb-8 flex-wrap gap-4">
            <div>
              ...
              <p className="text-gray-500 text-sm">Gerencie os materiais cadastrados no estoque.</p>
            </div>

            <button onClick={() => navigate('/admin/cadastrar-material')} className="bg-red-600 hover:bg-red-700 text-white px-5 py-3 rounded-xl font-bold text-sm transition-all">
              + Novo Material
            </button>
          </div>
          ...
          <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm mb-6 flex flex-wrap gap-4 items-end">
            <div className="flex-1 min-w-[300px]">
              ...
              <div className="relative">
                <input type="text" placeholder="Nome, código ou categoria..." className="w-full pl-4 pr-10 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-red-100 focus:border-red-500 transition-all" />
                  ...
                  <tr key={item.code} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-center">
                      <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase ${
                        item.qty <= item.min ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'
                      }`}>
                        {item.qty <= item.min ? 'Baixo' : 'Em dia'}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex justify-center gap-3">
                        <button onClick={() => navigate('/admin/editar-material')}>
...
const materiaisData = [
  { code: 'MAT001', name: 'Luva de Proteção', category: 'EPI', qty: 45, min: 50 },
  { code: 'MAT002', name: 'Fita Isolante', category: 'Elétrica', qty: 12, min: 10 },
  { code: 'MAT003', name: 'Parafuso Sextavado', category: 'Fixação', qty: 500, min: 100 },
  { code: 'MAT004', name: 'Graxa Azul', category: 'Lubrificantes', qty: 4, min: 5 },
];
